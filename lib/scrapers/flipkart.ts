...
export async function scrapeFlipkart(searchTerm: string): Promise<SearchResultItem[]> {
    ...
    try {
        ...
        const products = await page.evaluate((baseUrl, selector, searchTerm) => {
            const results: SearchResultItem[] = [];
            const productElements = document.querySelectorAll(selector);
            const searchWords = searchTerm.toLowerCase().split(' ').filter(word => word.length > 2);

            productElements.forEach((element) => {
                if (results.length >= 15) return; // Get more for better filtering
            ...
            return results
                ...
                .slice(0, 10);
        }, baseUrl, usedSelector, searchTerm);
